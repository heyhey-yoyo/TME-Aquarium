...
工作台使用 `ydchen-portfolio` 的米白 / 赤陶色视觉系统（页面底色 `#f3eee5`、正文 `#24221f`、赤陶强调 `#c15f3c`，QA 层强调色为对比度加深至 `#a94f31`）；模拟画布保持深色独立令牌。视觉调整不得改变空间模拟、画布语义、存档迁移、证据登记、Service Worker 清单或 CSP。
...
键盘与鼠标复用 inspectCanvasAt；十字准星属于渲染状态，不能写入模拟快照。重置、失焦和清除探针时清理准星。浅色工作台与深色画布分域，更新静态资源时同步提升 sw.js 缓存名。

## 问题闭环维护

stage-card 的时间轴行使用 auto，事件卡允许标题换行、说明采用明确行高，不能再把底行固定为 100px。修改后检查 390px、820px、1440px 与低高度窗口，确认事件卡底缘位于容器内；同步提升离线缓存名。
...
品牌页眉位于文档顶部，随页面正常滚走，不使用 fixed/sticky 吸顶；字体、字号、标志尺寸和三类排布基准保持一致。 桌面参数和结果使用整页滚动；画布保留合理高度，初始化时间轴完整显示。
